...
class MortgageModel:
    ...
    def simulate(self, steps=100, policy=None):
        """
        Simulate the system for a certain number of steps using the given policy.
        :param steps: Number of simulation steps
        :param policy: A function that returns the next action
        :return: Total reward over all steps
        """
        total_reward = 0
        for _ in range(steps):
            action = policy(self.state)  # Get action from the policy
            # Simulating exogenous factors (house value change, credit change, income change)
            exogenous_info = (
            np.random.uniform(-0.02, 0.02), np.random.randint(-10, 10), np.random.randint(-2000, 2000))
            self.transition(action, exogenous_info)
            r = self.reward(action)
            total_reward += r
        return total_reward2
